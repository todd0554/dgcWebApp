...
const PostsEvent = () => {
  ...
  const imageUrls = useMemo(() => {
    ...
    // normalize + filter invalid + newest first
    return arr
      .map(normalizeUrl)
      .filter((u) => !!u)
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={3} mb={4} flexWrap="wrap">
        <Input id="eventImageInput" type="file" hidden onChange={handleUpload} />
        <Button
          as="label"
        ...
        <Text fontSize="sm" color="gray.600">
          Event Gallery • Newest first • JPG / PNG only
        </Text>
      </Box>
